...
class CustomBannerController extends Controller
{
    public function index()
    {
        ...
        $latestBanners = [];
        $directory = storage_path('app/public/custom-banners');
        if (is_dir($directory)) {
        ...
        return view('custom.banner', compact('flagColors', 'borderColors', 'textColors', 'fonts', 'latestBanners'));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $filename = 'custom-banners/cb-' . time() . '-' . uniqid() . '.webp';
        file_put_contents(storage_path('app/public/' . $filename), $imageData);

        // Build cart entry
        $cart = session()->get('cart', []);
        $cartKey = 'custom-banner-' . time() . '-' . uniqid();
        ...
        return response()->json(['success' => true, 'message' => 'Custom banner added to cart!']);
    }
}
